...
const NumberBall = ({ number, type = 'winning' }) => (
    <div className={`number-ball ${type === 'winning' ? 'winning' : 'machine'}`}>
        {number}
    </div>
);
        ...
        <div className="result-card">
            <div className="result-card-header">
                <div>
                    ...
                    <p className="result-card-meta">Draw #{result.draw_number} • Machine {result.machine}</p>
                </div>
                <span className="result-card-time">{result.draw_time}</span>
            </div>
